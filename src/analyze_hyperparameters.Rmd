...
df_svm_male %>% ggplot(aes(x=C, y=ROC, color=as.factor(sigma))) + 
  ...
  geom_point(mapping=aes(x=C, ROC), data=best_svm_male, color="black", fill=NA, size=5, shape=1)
df_svm_female %>% ggplot(aes(x=C, y=ROC, color=as.factor(sigma))) + 
  geom_line() + 
  geom_point() + 
  scale_x_log10(breaks=svm_breaks) + 
  labs(title="Female", color="Sigma") +
  geom_point(mapping=aes(x=C, ROC), data=best_svm_female, color="black", fill=NA, size=5, shape=1)
```
...
nodesize_label <- get_label(rf_male_hyper, "nodesize")
...
df_rf_male %>% ggplot(aes(x=nodesize, y=ROC, color=as.factor(mtry))) + 
  geom_line() + 
  geom_point() + 
  #scale_x_log10()+#breaks=rf_breaks) + 
  labs(title="Male", x=nodesize_label, color=mtry_label) + 
  geom_point(mapping=aes(x=nodesize, ROC), data=best_rf_male, color="black", fill=NA, size=5, shape=1)
df_rf_female %>% ggplot(aes(x=nodesize, y=ROC, color=as.factor(mtry))) + 
  geom_line() + 
  geom_point() + 
  #scale_x_log10(breaks=rf_breaks) + 
  labs(title="Female", x=nodesize_label, color=mtry_label) +
  geom_point(mapping=aes(x=nodesize, ROC), data=best_rf_female, color="black", fill=NA, size=5, shape=1)
```
